...
class ROSFMUWrapper {
 public:
  ...
  explicit ROSFMUWrapper(const std::string& fmuPath, ros::Duration stepSize = ros::Duration(0.001),
                         bool interpolateInput = true, const std::string& tmpPath = "");

  ROSFMUWrapper(const ROSFMUWrapper& other) = delete;

  ROSFMUWrapper& operator=(const ROSFMUWrapper&) = delete;

  virtual ~ROSFMUWrapper();
